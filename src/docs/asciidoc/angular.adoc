...
<**span class="help-block" *ngIf="_email_.touched && !_email_.valid"**>Please enter a valid email!</**span**>
...
<**p *ngIf="_userData_.touched && !_userData_.valid"**>User data is invalid</**p**>
...
*'username'*: *new* FormControl(*null*, [Validators._required_, *this*.forbiddenNames.bind(*this*)]),

*'email'*: *new* FormControl(*null*, [Validators._required_, Validators._email_], *this*.forbiddenEmails)
...
*const* control = *new* FormControl(*null*, Validators._required_);
...
**return new _Promise_**<**any**>(((resolve) => {
