...
CloudHSM - dedicated hardware encryption module.

Custom key stores - interacting with CloudHSM on your behalf.

=== Elastic Beanstalk

Platform as a Service product.

Handles provisioning, monitoring, Auto Scaling, load balancing, and software updating.
...
You need to alter application code to be able to use Elastic Beanstalk.

Don't use Elastic Beanstalk when:

* you want low-level infrastructure control
* need Chef support
...
Moves away from the low-level configuration
of CloudFormation but nos as far as Elastic Beanstalk.

Lets you create a stack of resources with layers and manage
resources as a unit.
...
* stacks
  ...
  ** can use stacks per application or per plaftorm
  ** could use stacks for development, staging, or prod environments
* layers
  ** comparable to application ties within a stack
  ** e.g., database, application, proxy layer
  ** recipes are generally associated with layers and configure what to
     install on instances in that layer
* instances
...
* apps
  ...
  ** other recipes are run when deployments happen, potentially
     to reconfigure other instances.
* recipes
  ** setup - executed on an instance when first provisioned
  ** configure - executed on all instances when instance is added
     or removed
  ** deploy and undeploy - executed when apps are added or removed
  ** shutdown - executed when an instance is shut down but before it's
     stopped

Chef 11 - linux only

Chef 12 - linux and windows
